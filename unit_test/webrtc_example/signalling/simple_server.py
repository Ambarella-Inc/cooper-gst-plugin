...
class WebRTCSimpleServer(object):
    ...
    def __init__(self, options):
        ...
        self.sdp_delay = options.sdp_delay
    ...
    def _is_sdp_offer(self, msg):
        """Check if message is a JSON SDP offer"""
        try:
            data = json.loads(msg)
            if isinstance(data, dict) and 'sdp' in data:
                sdp = data['sdp']
                if isinstance(sdp, dict) and sdp.get('type') == 'offer':
                    return True
    ...
    async def cleanup_session(self, uid):
        if uid in self.sessions:
            ...
            if other_id in self.sessions:
                ...
                # instead of forcibly closing their connection
                if other_id in self.peers:
                    print("Notifying {} that peer {} disconnected".format(other_id, uid))
                    wso, oaddr, _ = self.peers[other_id]
                    # Reset peer status so they can start a new session
                    self.peers[other_id][2] = None
                    try:
                        await wso.send('PEER_DISCONNECTED {}'.format(uid))
                    except Exception as e:
                        print("Failed to notify {}: {}".format(other_id, e))
        ...
        while True:
            ...
            if peer_status is not None:
                ...
                if peer_status == 'session':
                    ...
                    # Add delay for SDP offer messages to allow transceiver caps to be ready
                    if self.sdp_delay > 0 and self._is_sdp_offer(msg):
                        print("Delaying SDP offer by {}ms to allow transceiver preparation".format(self.sdp_delay))
                        await asyncio.sleep(self.sdp_delay / 1000.0)
                    await wso.send(msg)
    ...
    async def run(self):
        async def handler(ws):
            ...
            Note: websockets 15.0+ removed the 'path' parameter from handler signature.
            '''
            raddr = ws.remote_address
        ...
        protocol = "https" if sslctx else "http"
        print("Listening on {}://{}:{}".format(protocol, self.addr if self.addr else "0.0.0.0", self.port))
        # Websocket server
...
def main():
    ...
    parser.add_argument('--sdp-delay', default=0, type=int, help='Delay in milliseconds before forwarding SDP offer messages (helps with transceiver caps race condition)')
