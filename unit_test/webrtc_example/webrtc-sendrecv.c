 ...
 *
 * History:
 *    01/14/2025 - [Yang Yu] created file
 *
...
static gboolean send_only = FALSE;
static gboolean recv_only = FALSE;
static guint stats_timer_id = 0;

static const gchar *test_video_desc = "videotestsrc is-live=true pattern=ball ! videoconvert ! queue ! "
                                      "vp8enc deadline=1 keyframe-max-dist=2000 ! "
                                      "rtpvp8pay name=videopay picture-id-mode=15-bit pt=96 ! queue";

static const gchar *test_audio_desc = "audiotestsrc is-live=true wave=red-noise ! audioconvert ! audioresample"
                                      "! queue ! opusenc perfect-timestamp=true ! rtpopuspay name=audiopay pt=97 "
...
static GOptionEntry entries[] = {
    ...
    {"video_desc", 0, 0, G_OPTION_ARG_STRING, &test_video_desc,
     "Video pipeline to send", "PIPELINE"},
    {"audio_desc", 0, 0, G_OPTION_ARG_STRING, &test_audio_desc,
     "Audio pipeline to send", "PIPELINE"},
    {"disable-audio", 0, 0, G_OPTION_ARG_NONE, &disable_audio, "Disable audio (no send, use fakesink for recv)", NULL},
    {"disable-video", 0, 0, G_OPTION_ARG_NONE, &disable_video, "Disable video display (use fakesink for recv)", NULL},
    ...
    {"send-only", 0, 0, G_OPTION_ARG_NONE, &send_only,
     "Only send media, do not receive", NULL},
    {"recv-only", 0, 0, G_OPTION_ARG_NONE, &recv_only,
     "Only receive media, do not send", NULL},
    {NULL},
};
...
/* Background thread function to clean up pipeline without blocking main loop */
static gpointer
cleanup_pipeline_thread(gpointer data)
{
  GstElement *pipeline = GST_ELEMENT(data);

  gst_println("Background: Setting pipeline to NULL state...");
  gst_element_set_state(pipeline, GST_STATE_NULL);
  gst_println("Background: Pipeline stopped, unreferencing...");
  gst_object_unref(pipeline);
  gst_println("Background: Pipeline cleanup complete");

  return NULL;
}

/* Stop stats timer and prepare for pipeline cleanup */
static void
...
on_incoming_stream(GstElement *webrtc, GstPad *pad, GstElement *pipe)
{
  ...
  /* In send-only mode, we should not receive any streams.
   * If we do, the peer is misconfigured - they should use recv-only mode. */
  if (send_only)
  {
    gst_printerr("ERROR: Received incoming stream in send-only mode.\n");
    gst_printerr("The peer should use --recv-only mode when connecting to a send-only sender.\n");
    cleanup_and_quit_loop("Peer not in recv-only mode", PEER_CALL_ERROR);
    return;
  }
...
on_negotiation_needed(GstElement *element, gpointer user_data)
{
  ...
  /* In recv-only mode, we should not create offers - we wait for the sender
   * to create the offer. Same behavior as remote_is_offerer. */
  if (remote_is_offerer || recv_only)
  {
...
data_channel_on_close(GObject *dc, gpointer user_data)
{
  ...
  gst_println("Data channel closed");

  /* Don't exit if we're waiting for a new connection */
  if (our_id && app_state == SERVER_REGISTERED)
  {
    gst_println("Staying connected, waiting for new peer");
    return;
  }

  /* Only exit if we're not in a state where we expect to wait */
...
on_webrtcbin_get_stats(GstPromise *promise, GstElement *webrtcbin)
{
  ...
  /* Check if pipeline was cleaned up */
  if (!webrtc1 || webrtcbin != webrtc1)
  {
    gst_promise_unref(promise);
    return;
  }
  ...
  /* Schedule next stats query only if pipeline is still valid */
  if (webrtc1)
  {
    stats_timer_id = g_timeout_add(100, (GSourceFunc)webrtcbin_get_stats, webrtcbin);
  }
}
...
#define RTP_AUDIO_DEFAULT_PT 97
#define RTP_VIDEO_DEFAULT_PT 96

static gboolean
start_pipeline(gboolean create_offer, guint audio_pt, guint video_pt)
{
  GstBus *bus;
  ...
  /* In recv-only mode, we don't need to create send pipelines */
  if (!recv_only)
  {
    /* Only create audio pipeline if audio is not disabled */
    if (!disable_audio)
    {
#if 0
      audio_desc =
          g_strdup_printf("audiotestsrc is-live=true wave=red-noise ! audioconvert ! audioresample"
                          "! queue ! opusenc perfect-timestamp=true ! rtpopuspay name=audiopay pt=%u "
                          "! application/x-rtp, encoding-name=OPUS ! queue",
                          audio_pt);
#endif
      audio_bin = gst_parse_bin_from_description(test_audio_desc, TRUE, &audio_error);
      if (audio_error)
      {
  ...
  video_desc =
      ...
      "rtpvp8pay name=videopay picture-id-mode=15-bit pt=%u ! queue", video_pt);
#endif
  ...
  /* In recv-only mode, we don't create send pipelines, just add recvonly transceivers */
  if (recv_only)
  {
    gst_print("Recv-only mode: pipeline will be configured when receiving offer\n");
    gst_bin_add(GST_BIN(pipe1), webrtc1);
    /*
     * In recv-only mode, we don't add transceivers here.
     * When we receive an offer from the remote peer, the on_offer_received()
     * function will parse the SDP to determine the encoding formats,
     * and webrtcbin will automatically create the appropriate transceivers
     * based on the remote offer.
     */
  }
  else
  {
    video_bin = gst_parse_bin_from_description(test_video_desc, TRUE, &video_error);
    ...
    /* Takes ownership of each - add audio_bin only if not disabled */
    if (!disable_audio && audio_bin)
    {
    ...
    {
      gst_bin_add_many(GST_BIN(pipe1), video_bin, webrtc1, NULL);
    }
  ...
  {
    /* In recv-only mode, we don't have videopay/audiopay elements */
    GstElement *videopay;
    ...
    /* Only set up audio TWCC if audio is not disabled */
    if (!disable_audio)
    {
      GstElement *audiopay;
      GstRTPHeaderExtension *audio_twcc;
  ...
  stats_timer_id = g_timeout_add(100, (GSourceFunc)webrtcbin_get_stats, webrtc1);
...
on_offer_received(GstSDPMessage *sdp)
{
   ...
   * This happens when:
   * 1. Using --our-id (waiting for connection, peer sends offer)
   * 2. Using --peer-id with --recv-only (we requested OFFER_REQUEST, peer sends offer)
   */
  if (!webrtc1 && (our_id || recv_only))
  {
    guint medias_len, formats_len;
    guint audio_pt = 0, video_pt = 0;

    gst_println("Parsing offer to find payload types");

    medias_len = gst_sdp_message_medias_len(sdp);
    for (guint i = 0; i < medias_len; i++)
    {
      const GstSDPMedia *media = gst_sdp_message_get_media(sdp, i);
      formats_len = gst_sdp_media_formats_len(media);
      for (guint j = 0; j < formats_len; j++)
      {
        guint pt;
        GstCaps *caps;
        GstStructure *s;
        ...
        if (video_pt == 0 && g_strcmp0(encoding_name, "VP8") == 0)
        {
          video_pt = pt;
        }
        if (video_pt == 0 && g_strcmp0(encoding_name, "H264") == 0)
        {
          video_pt = pt;
        }
        if (video_pt == 0 && g_strcmp0(encoding_name, "H265") == 0)
        {
          video_pt = pt;
        }
        if (audio_pt == 0 && g_strcmp0(encoding_name, "OPUS") == 0)
        {
          audio_pt = pt;
        }
      }
    ...
    /* In send-only mode, we typically shouldn't receive an offer with media.
     * If we do, it might be from a misconfigured peer. Handle gracefully. */
    if (send_only && audio_pt == 0 && video_pt == 0)
    {
      gst_printerr("Warning: Received offer without media in send-only mode. "
                   "The peer should use 'Remote offerer' or recv-only mode.\n");
      return;
    }

    /* Validate that we have the required media types based on configuration.
     * When disable_audio or disable_video is set, we don't require that media type.
     * In recv-only or send-only mode, we also relax requirements. */
    if (!recv_only && !send_only)
    {
      if (!disable_audio && audio_pt == 0)
      {
        gst_printerr("Warning: No audio in offer, but audio is not disabled. "
                     "Use --disable-audio if intentional.\n");
      }
      if (!disable_video && video_pt == 0)
      {
        gst_printerr("Warning: No video in offer, but video is not disabled. "
                     "Use --disable-video if intentional.\n");
      }
    }

    /* Need at least one media type */
    if (audio_pt == 0 && video_pt == 0)
    {
      gst_printerr("Error: No media types found in offer\n");
      return;
    }

    gst_println("Starting pipeline with audio pt: %u video pt: %u", audio_pt,
                video_pt);

    if (!start_pipeline(FALSE, audio_pt, video_pt))
    {
                  ...
                  GBytes *message, gpointer user_data)
{
  ...
  else if (g_strcmp0(text, "SESSION_OK") == 0)
  {
    ...
    /* In recv-only mode, we should ask the sender to create the offer,
     * because we don't have any send pipeline to generate media descriptions */
    if (recv_only)
    {
      gst_print("Recv-only mode: requesting remote peer to send offer\n");
      soup_websocket_connection_send_text(ws_conn, "OFFER_REQUEST");
    }
    else
    {
      /* Start negotiation (exchange SDP and ICE candidates) */
      if (!start_pipeline(TRUE, RTP_AUDIO_DEFAULT_PT, RTP_VIDEO_DEFAULT_PT))
        cleanup_and_quit_loop("ERROR: failed to start pipeline",
                              PEER_CALL_ERROR);
    }
  }
  ...
  else if (g_str_has_prefix(text, "PEER_DISCONNECTED"))
  {
    /* Peer has disconnected, but we stay connected to the server */
    gst_println("Peer disconnected");

    /* Stop stats timer first to prevent accessing freed objects */
    stop_stats_timer();

    /* Clean up current pipeline in a background thread to avoid blocking */
    if (pipe1)
    {
      GstElement *old_pipe = pipe1;
      pipe1 = NULL;
      webrtc1 = NULL;

      /* Use a thread to clean up pipeline so we don't block */
      g_thread_new("pipeline-cleanup", (GThreadFunc)cleanup_pipeline_thread, old_pipe);
      gst_println("Pipeline cleanup started in background");
    }

    /* If we have our-id, we can wait for a new connection */
    if (our_id)
    {
      app_state = SERVER_REGISTERED;
      gst_println("Waiting for new connection (our-id: %s)", our_id);
    }
    else
    {
      /* We initiated the call, so just exit */
      gst_println("No our-id set, exiting");
      cleanup_and_quit_loop("Peer disconnected", 0);
    }
  }
...
#else
  soup_session_websocket_connect_async(session, message, NULL, NULL, 0, NULL,
                                       (GAsyncReadyCallback)on_server_connected, message);
#endif
...
int main(int argc, char *argv[])
{
  ...
  g_option_context_set_description(context,
      "Examples:\n"
      "  Wait for a peer to connect (as sender):\n"
      "    $ test-webrtc-sendrecv --our-id=1234 --send-only\n"
      "\n"
      "  Wait for a peer to connect (as receiver):\n"
      "    $ test-webrtc-sendrecv --our-id=1234 --recv-only\n"
      "\n"
      "  Connect to a specific peer (as sender):\n"
      "    $ test-webrtc-sendrecv --peer-id=5678 --send-only\n"
      "\n"
      "  Connect to a specific peer (as receiver):\n"
      "    $ test-webrtc-sendrecv --peer-id=5678 --recv-only\n"
      "\n"
      "  Send video only (no audio):\n"
      "    $ test-webrtc-sendrecv --our-id=1234 --send-only --disable-audio\n"
      "\n"
      "  Use amba_venccap2 element:\n"
      "    $ test-webrtc-sendrecv --our-id=1234 --send-only --disable-audio \\\n"
      "        --video_desc=\"amba_venccap2 stream-id=0 ! queue ! h264parse ! rtph264pay name=videopay pt=96\"\n"
      "\n"
      "  Connect to a custom server:\n"
      "    $ test-webrtc-sendrecv --our-id=1234 --server=ws://10.0.0.2:8443\n"
      "\n"
      "Notes:\n"
      "  - Use --our-id to wait for incoming connections\n"
      "  - Use --peer-id to initiate a connection to a specific peer\n"
      "  - In recv-only mode, the remote peer must be the offerer (create the SDP offer)\n"
      "  - Default server is ws://127.0.0.1:8443\n");
  ...
  {
    gst_printerr("--send-only and --recv-only cannot be used together\n");
    goto out;
  }

  if (send_only)
    gst_print("Running in send-only mode\n");
  if (recv_only)
    gst_print("Running in recv-only mode\n");
