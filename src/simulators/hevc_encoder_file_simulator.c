...
} hevc_enc_file_simulator_ctx_t;

static void __clean_hevc_encoder_file_simulator(
  hevc_enc_file_simulator_ctx_t * thiz)
{
  if (thiz) {
    if (thiz->p_file) {
      fclose(thiz->p_file);
...
static void destroy_hevc_encoder_file_simulator(
  enc_simulator_base_t * ctx)
{
  __clean_hevc_encoder_file_simulator((hevc_enc_file_simulator_ctx_t * ) ctx);
}
...
{
  hevc_enc_file_simulator_ctx_t * thiz = (hevc_enc_file_simulator_ctx_t * ) ctx;
  unsigned int next_size;
  unsigned char nal_type = 0, need_more_data = 0, reach_tail = 0;

  next_size = hevc_get_next_frame(thiz->p_cur,
  ...
  if (next_size) {
    ...
    *p_size = next_size;
    ...
    if ((EHEVCNalType_IDR_W_RADL == nal_type)
      || (EHEVCNalType_IDR_N_LP == nal_type)) {
...
{
  hevc_enc_file_simulator_ctx_t * thiz = NULL;
  int failed = 0;
  int ret = 0;

  do {
    thiz = malloc (sizeof(hevc_enc_file_simulator_ctx_t));
    if (!thiz) {
      DPRINT_ERROR("no memory\n");
      failed = 1;
      break;
    }
    memset (thiz, 0x0, sizeof(hevc_enc_file_simulator_ctx_t));
  ...
  if (failed) {
    __clean_hevc_encoder_file_simulator(thiz);
    return NULL;
  } else {
    thiz->base.f_encode_sim = encode_sim;
    thiz->base.f_destroy = destroy_hevc_encoder_file_simulator;
  }
