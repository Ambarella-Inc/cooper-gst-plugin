...
GST_DEBUG_CATEGORY_STATIC (msplitmux_debug);
#define GST_CAT_DEFAULT msplitmux_debug

#define GST_MSPLITMUX_STATE_LOCK(s) g_mutex_lock(&(s)->state_lock)
#define GST_MSPLITMUX_STATE_UNLOCK(s) g_mutex_unlock(&(s)->state_lock)

#define GST_MSPLITMUX_LOCK(s) g_mutex_lock(&(s)->lock)
#define GST_MSPLITMUX_UNLOCK(s) g_mutex_unlock(&(s)->lock)
#define GST_MSPLITMUX_WAIT_INPUT(s) g_cond_wait (&(s)->input_cond, &(s)->lock)
#define GST_MSPLITMUX_BROADCAST_INPUT(s) g_cond_broadcast (&(s)->input_cond)

#define GST_MSPLITMUX_WAIT_OUTPUT(s) g_cond_wait (&(s)->output_cond, &(s)->lock)
#define GST_MSPLITMUX_BROADCAST_OUTPUT(s) g_cond_broadcast (&(s)->output_cond)

/* When deciding whether to suppress a ForceKeyUnit (non-timecode path), compare
 * candidate target_time = gop_start + threshold against next_fku_time. Hardware
 * encoders often emit IDR slightly before the requested running_time; then
 * gop_start + threshold undershoots next_fku_time by tens of ms and the next
 * FKU is wrongly deferred by a full GOP. Timecode mode already uses a small
 * rounding slack; this is the live-clock analogue. */
 #define MSPLITMUX_FKU_NEXT_MIN_INTERVAL_SLACK (150 * GST_MSECOND)

static void split_now (GstMSplitMuxSink * splitmux);
static void split_after (GstMSplitMuxSink * splitmux);
static void split_at_running_time (GstMSplitMuxSink * splitmux,
...
_do_init (void)
{
  PAD_CONTEXT = g_quark_from_static_string ("msplitmuxsink-pad-context");
  EOS_FROM_US = g_quark_from_static_string ("msplitmuxsink-eos-from-us");
  SINK_FRAGMENT_INFO =
      g_quark_from_static_string ("msplitmuxsink-fragment-info");
  GST_DEBUG_CATEGORY_INIT (msplitmux_debug, "msplitmuxsink", 0,
      "Modified Split File Muxing Sink");
}

#define gst_msplitmux_sink_parent_class parent_class
G_DEFINE_TYPE_EXTENDED (GstMSplitMuxSink, gst_msplitmux_sink, GST_TYPE_BIN, 0,
    _do_init ());
GST_ELEMENT_REGISTER_DEFINE (msplitmuxsink, "msplitmuxsink", GST_RANK_NONE,
    GST_TYPE_MSPLITMUX_SINK);

static gboolean create_muxer (GstMSplitMuxSink * splitmux);
static gboolean create_sink (GstMSplitMuxSink * splitmux);
static void gst_msplitmux_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_msplitmux_sink_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static void gst_msplitmux_sink_dispose (GObject * object);
static void gst_msplitmux_sink_finalize (GObject * object);

static GstPad *gst_msplitmux_sink_request_new_pad (GstElement * element,
    GstPadTemplate * templ, const gchar * name, const GstCaps * caps);
static void gst_msplitmux_sink_release_pad (GstElement * element, GstPad * pad);

static GstStateChangeReturn gst_msplitmux_sink_change_state (GstElement *
    element, GstStateChange transition);

static void bus_handler (GstBin * bin, GstMessage * msg);
static void set_next_filename (GstMSplitMuxSink * splitmux, MqStreamCtx * ctx);
static GstFlowReturn start_next_fragment (GstMSplitMuxSink * splitmux,
    MqStreamCtx * ctx);
static void mq_stream_ctx_free (MqStreamCtx * ctx);
static void grow_blocked_queues (GstMSplitMuxSink * splitmux);

static void gst_msplitmux_sink_ensure_max_files (GstMSplitMuxSink * splitmux);
static GstElement *create_element (GstMSplitMuxSink * splitmux,
    const gchar * factory, const gchar * name, gboolean locked);

static void do_async_done (GstMSplitMuxSink * splitmux);

static GstClockTime calculate_next_max_timecode (GstMSplitMuxSink * splitmux,
...
static MSplitMuxOutputCommand *
out_cmd_buf_new_finish_fragment (void)
{
  MSplitMuxOutputCommand *res = g_new0 (MSplitMuxOutputCommand, 1);

  res->cmd_type = MSPLITMUX_OUTPUT_COMMAND_FINISH_FRAGMENT;
  return res;
}
...
out_cmd_buf_new_release_gop (GstClockTimeDiff max_output_ts)
{
  ...
  res->cmd_type = MSPLITMUX_OUTPUT_COMMAND_RELEASE_GOP;
...
static void
out_cmd_buf_free (MSplitMuxOutputCommand * data)
{
  g_free (data);
}
...
static void
gst_msplitmux_sink_class_init (GstMSplitMuxSinkClass * klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstElementClass *gstelement_class = (GstElementClass *) klass;
  GstBinClass *gstbin_class = (GstBinClass *) klass;

  gobject_class->set_property = gst_msplitmux_sink_set_property;
  gobject_class->get_property = gst_msplitmux_sink_get_property;
  gobject_class->dispose = gst_msplitmux_sink_dispose;
  gobject_class->finalize = gst_msplitmux_sink_finalize;
  ...
  gst_element_class_set_static_metadata (gstelement_class,
      ...
      "Modified by Ambarella, will delete old files instead of overwriting when max-files is reached.",
  ...
  gstelement_class->change_state =
      GST_DEBUG_FUNCPTR (gst_msplitmux_sink_change_state);
  gstelement_class->request_new_pad =
      GST_DEBUG_FUNCPTR (gst_msplitmux_sink_request_new_pad);
  gstelement_class->release_pad =
      GST_DEBUG_FUNCPTR (gst_msplitmux_sink_release_pad);
  ...
  /**
   * GstMSplitMuxSink::muxer-pad-map
   *
  ...
  /**
   * GstMSplitMuxSink::format-location:
   * @splitmux: the #GstMSplitMuxSink
  ...
  /**
   * GstMSplitMuxSink::format-location-full:
   * @splitmux: the #GstMSplitMuxSink
   * @fragment_id: the sequence number of the file to be created
  ...
  signals[SIGNAL_SPLIT_NOW] =
      ...
      G_STRUCT_OFFSET (GstMSplitMuxSinkClass, split_now), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  /**
   * GstMSplitMuxSink::split-after:
   * @splitmux: the #GstMSplitMuxSink
   *
   * When called by the user, this action signal splits the video file (and begins a new one) immediately.
  ...
  signals[SIGNAL_SPLIT_AFTER] =
      ...
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET (GstMSplitMuxSinkClass, split_after), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  /**
   * GstMSplitMuxSink::split-at-running-time:
   * @splitmux: the #GstMSplitMuxSink
   *
  ...
  signals[SIGNAL_SPLIT_AT_RUNNING_TIME] =
      ...
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET (GstMSplitMuxSinkClass, split_at_running_time), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_UINT64);

  /**
   * GstMSplitMuxSink::muxer-added:
   * @splitmux: the #GstMSplitMuxSink
...
static void
gst_msplitmux_sink_init (GstMSplitMuxSink * splitmux)
{
  ...
  g_queue_init (&splitmux->old_files);
...
static void
gst_splitmux_reset_elements (GstMSplitMuxSink * splitmux)
{
  if (splitmux->muxer) {
...
static void
gst_msplitmux_sink_dispose (GObject * object)
{
  GstMSplitMuxSink *splitmux = GST_MSPLITMUX_SINK (object);
...
static void
gst_msplitmux_sink_finalize (GObject * object)
{
  GstMSplitMuxSink *splitmux = GST_MSPLITMUX_SINK (object);
  ...
  g_queue_foreach (&splitmux->old_files, (GFunc) g_free, NULL);
  g_queue_clear (&splitmux->old_files);
 ...
 * in create_elements() */
static void
update_muxer_properties (GstMSplitMuxSink * sink)
{
  GObjectClass *klass;
...
static void
gst_msplitmux_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstMSplitMuxSink *splitmux = GST_MSPLITMUX_SINK (object);
...
static void
gst_msplitmux_sink_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstMSplitMuxSink *splitmux = GST_MSPLITMUX_SINK (object);
...
static MqStreamCtx *
mq_stream_ctx_new (GstMSplitMuxSink * splitmux)
{
  MqStreamCtx *ctx;
...
static void
send_fragment_opened_closed_msg (GstMSplitMuxSink * splitmux, gboolean opened,
    GstElement * sink)
{
  gchar *location = NULL;
...
static void
send_eos_async (GstMSplitMuxSink * splitmux, AsyncEosHelper * helper)
{
  GstEvent *eos;
  GstPad *pad;
  MqStreamCtx *ctx;
 ...
 */
static void
send_eos (GstMSplitMuxSink * splitmux, MqStreamCtx * ctx)
{
  GstEvent *eos;
  GstPad *pad;
...
static void
eos_context_async (MqStreamCtx * ctx, GstMSplitMuxSink * splitmux)
{
...
static gboolean
all_contexts_are_async_eos (GstMSplitMuxSink * splitmux)
{
  gboolean ret = TRUE;
  GList *item;
...
static void
update_output_fragment_info (GstMSplitMuxSink * splitmux)
{
...
complete_or_wait_on_out (GstMSplitMuxSink * splitmux, MqStreamCtx * ctx)
{
  if (ctx->caps_change)
    return GST_FLOW_OK;
  ...
  do {
    ...
    if (ctx->flushing
        || splitmux->output_state == MSPLITMUX_OUTPUT_STATE_STOPPED)
    ...
    if (can_output) {
      ...
      switch (splitmux->output_state) {
        case MSPLITMUX_OUTPUT_STATE_OUTPUT_GOP:
          /* We only get here if we've finished outputting a GOP and need to know
           * what to do next */
          splitmux->output_state = MSPLITMUX_OUTPUT_STATE_AWAITING_COMMAND;
          GST_MSPLITMUX_BROADCAST_OUTPUT (splitmux);
          continue;

        case MSPLITMUX_OUTPUT_STATE_ENDING_FILE:
        case MSPLITMUX_OUTPUT_STATE_ENDING_STREAM:
            ...
            if (splitmux->async_finalize) {
              ...
              *sink_fragment_info = splitmux->out_fragment_info;
              ...
              if (all_contexts_are_async_eos (splitmux)) {
                ...
                splitmux->output_state = MSPLITMUX_OUTPUT_STATE_START_NEXT_FILE;
                GST_MSPLITMUX_BROADCAST_OUTPUT (splitmux);
                continue;
              }
            } else {
              ...
              MSPLITMUX_OUTPUT_STATE_ENDING_STREAM) {
            GST_LOG_OBJECT (splitmux,
          ...
          break;
        case MSPLITMUX_OUTPUT_STATE_START_NEXT_FILE:
          if (ctx->is_reference) {
          ...
          break;
        case MSPLITMUX_OUTPUT_STATE_AWAITING_COMMAND:{
          do {
            MSplitMuxOutputCommand *cmd =
            ...
            if (cmd != NULL) {
              ...
              switch (cmd->cmd_type) {
                case MSPLITMUX_OUTPUT_COMMAND_FINISH_FRAGMENT:
                {
                  if (splitmux->muxed_out_bytes > 0) {
                    GST_DEBUG_OBJECT (splitmux,
                        "Got cmd to start new fragment");
                    splitmux->output_state = MSPLITMUX_OUTPUT_STATE_ENDING_FILE;
                  } else {
                    GST_DEBUG_OBJECT (splitmux,
                ...
                case MSPLITMUX_OUTPUT_COMMAND_RELEASE_GOP:
                {
                  ...
                  GST_DEBUG_OBJECT (splitmux,
                      ...
                      GST_STIME_ARGS (splitmux->max_out_running_time));
                  splitmux->output_state = MSPLITMUX_OUTPUT_STATE_OUTPUT_GOP;
                  break;
                }
                default:
              ...
              GST_MSPLITMUX_WAIT_OUTPUT (splitmux);
            }
          } while (!ctx->flushing && splitmux->output_state ==
              MSPLITMUX_OUTPUT_STATE_AWAITING_COMMAND);
          /* loop and re-check the state */
          continue;
        }
        case MSPLITMUX_OUTPUT_STATE_STOPPED:
          return GST_FLOW_FLUSHING;
      }
    } else {
...
static GstClockTime
calculate_next_max_timecode (GstMSplitMuxSink * splitmux,
    const GstVideoTimeCode * cur_tc, GstClockTime running_time,
    GstVideoTimeCode ** next_tc)
{
...
static gboolean
request_next_keyframe (GstMSplitMuxSink * splitmux, GstBuffer * buffer,
    GstClockTimeDiff running_time_dts)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (splitmux->next_fku_time)) {
    ...
    } else if (allowed_time > MSPLITMUX_FKU_NEXT_MIN_INTERVAL_SLACK) {
      allowed_time -= MSPLITMUX_FKU_NEXT_MIN_INTERVAL_SLACK;
    } else {
      allowed_time = 0;
    }
...
{
  GstMSplitMuxSink *splitmux = ctx->splitmux;
  ...
  if (info->type & GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM ||
    ...
    switch (GST_EVENT_TYPE (event)) {
      ...
      case GST_EVENT_FLUSH_START:
        ...
        ctx->flushing = TRUE;
        GST_MSPLITMUX_BROADCAST_INPUT (splitmux);
        GST_MSPLITMUX_BROADCAST_OUTPUT (splitmux);
        break;
      case GST_EVENT_EOS:
        GST_MSPLITMUX_LOCK (splitmux);
        locked = TRUE;
        if (splitmux->output_state == MSPLITMUX_OUTPUT_STATE_STOPPED)
          goto beach;
        ...
        if (ctx == splitmux->reference_ctx) {
          ...
          splitmux->output_state = MSPLITMUX_OUTPUT_STATE_ENDING_STREAM;
          update_output_fragment_info (splitmux);
      ...
      case GST_EVENT_CAPS:{
        ...
        if (splitmux->output_state != MSPLITMUX_OUTPUT_STATE_START_NEXT_FILE) {
          GST_DEBUG_OBJECT (splitmux,
...
static void
unlock_context (MqStreamCtx * ctx, GstMSplitMuxSink * splitmux)
{
...
static void
restart_context (MqStreamCtx * ctx, GstMSplitMuxSink * splitmux)
{
  GstPad *peer = gst_pad_get_peer (ctx->srcpad);
...
static void
relink_context (MqStreamCtx * ctx, GstMSplitMuxSink * splitmux)
{
...
static void
block_context (MqStreamCtx * ctx, GstMSplitMuxSink * splitmux)
{
...
static void
_lock_and_set_to_null (GstElement * element, GstMSplitMuxSink * splitmux)
{
 ...
 */
static GstFlowReturn
start_next_fragment (GstMSplitMuxSink * splitmux, MqStreamCtx * ctx)
{
  ...
  GST_MSPLITMUX_UNLOCK (splitmux);
  GST_MSPLITMUX_STATE_LOCK (splitmux);

  if (splitmux->shutdown) {
    GST_DEBUG_OBJECT (splitmux,
        "Shutdown requested. Aborting fragment switch.");
    GST_MSPLITMUX_LOCK (splitmux);
    GST_MSPLITMUX_STATE_UNLOCK (splitmux);
...
{
  GstMSplitMuxSink *splitmux = GST_MSPLITMUX_SINK (bin);
  ...
  switch (GST_MESSAGE_TYPE (message)) {
    case GST_MESSAGE_EOS:{
      ...
      } else if (splitmux->output_state == MSPLITMUX_OUTPUT_STATE_ENDING_STREAM) {
        GST_DEBUG_OBJECT (splitmux,
...
static gboolean
need_new_fragment (GstMSplitMuxSink * splitmux,
 ...
 */
static void
handle_gathered_gop (GstMSplitMuxSink * splitmux, const InputGop * gop,
    GstClockTimeDiff next_gop_start_time, GstClockTimeDiff max_out_running_time)
{
  ...
  if (max_out_running_time != G_MAXINT64) {
    splitmux->input_state = MSPLITMUX_INPUT_STATE_COLLECTING_GOP_START;
  } else {
    /* This is probably already the current state, but just in case: */
    splitmux->input_state = MSPLITMUX_INPUT_STATE_FINISHING_UP;
  }
 ...
 */
static void
check_completed_gop (GstMSplitMuxSink * splitmux, MqStreamCtx * ctx)
{
  GList *cur;
  GstEvent *event;
  ...
  if (ctx->need_unblock) {
    ...
    if (splitmux->input_state != MSPLITMUX_INPUT_STATE_WAITING_GOP_COLLECT)
      return;
  }
  ...
  do {
    ...
    if (splitmux->input_state == MSPLITMUX_INPUT_STATE_WAITING_GOP_COLLECT &&
      ...
      GST_MSPLITMUX_WAIT_INPUT (splitmux);
  ...
  } while (splitmux->input_state == MSPLITMUX_INPUT_STATE_WAITING_GOP_COLLECT);
}
...
handle_mq_input (GstPad * pad, GstPadProbeInfo * info, MqStreamCtx * ctx)
{
  ...
  if (info->type & GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM ||
    ...
    switch (GST_EVENT_TYPE (event)) {
      ...
      case GST_EVENT_EOS:
        ...
        if (splitmux->input_state == MSPLITMUX_INPUT_STATE_STOPPED) {
          ret = GST_FLOW_FLUSHING;
          goto beach;
        }
        ...
        if (ctx->is_reference) {
          ...
          splitmux->input_state = MSPLITMUX_INPUT_STATE_WAITING_GOP_COLLECT;
          ...
          if (g_queue_is_empty (&splitmux->pending_input_gops)) {
            ...
            MSPLITMUX_INPUT_STATE_WAITING_GOP_COLLECT) {
  ...
  do {
    ...
    switch (splitmux->input_state) {
      case MSPLITMUX_INPUT_STATE_COLLECTING_GOP_START:
        if (ctx->is_reference) {
          const InputGop *gop, *next_gop;
        ...
        break;
      case MSPLITMUX_INPUT_STATE_WAITING_GOP_COLLECT:{
      ...
      case MSPLITMUX_INPUT_STATE_FINISHING_UP:
        loop_again = FALSE;
        break;
      default:
        loop_again = FALSE;
   ...
   * EOS now */
  if (splitmux->input_state != MSPLITMUX_INPUT_STATE_FINISHING_UP
      && splitmux->input_state != MSPLITMUX_INPUT_STATE_STOPPED) {
    InputGop *gop = g_queue_peek_tail (&splitmux->pending_input_gops);
...
static void
grow_blocked_queues (GstMSplitMuxSink * splitmux)
{
  GList *cur;
...
/* Called with MSPLITMUX lock held */
static const gchar *
lookup_muxer_pad (GstMSplitMuxSink * splitmux, const gchar * sinkpad_name)
{
...
static GstPad *
gst_msplitmux_sink_request_new_pad (GstElement * element,
    GstPadTemplate * templ, const gchar * name, const GstCaps * caps)
{
  GstMSplitMuxSink *splitmux = (GstMSplitMuxSink *) element;
...
static void
gst_msplitmux_sink_release_pad (GstElement * element, GstPad * pad)
{
  GstMSplitMuxSink *splitmux = (GstMSplitMuxSink *) element;
  GstPad *muxpad = NULL;
  MqStreamCtx *ctx =
...
static GstElement *
create_element (GstMSplitMuxSink * splitmux,
    const gchar * factory, const gchar * name, gboolean locked)
{
...
static gboolean
create_muxer (GstMSplitMuxSink * splitmux)
{
...
static gboolean
create_sink (GstMSplitMuxSink * splitmux)
{
...
#endif
static void
set_next_filename (GstMSplitMuxSink * splitmux, MqStreamCtx * ctx)
{
  gchar *fname = NULL;
  GstSample *sample;
  GstCaps *caps;

  gst_msplitmux_sink_ensure_max_files (splitmux);
  ...
  if (fname) {
    ...
    if (splitmux->max_files > 0) {
      g_queue_push_tail (&splitmux->old_files, g_strdup (fname));
    }
    g_free (fname);
  }
...
/* called with GST_MSPLITMUX_LOCK */
static void
do_async_start (GstMSplitMuxSink * splitmux)
{
  GstMessage *message;
...
/* called with GST_MSPLITMUX_LOCK */
static void
do_async_done (GstMSplitMuxSink * splitmux)
{
  GstMessage *message;
...
static void
gst_msplitmux_sink_reset (GstMSplitMuxSink * splitmux)
{
...
gst_msplitmux_sink_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_READY_TO_PAUSED:{
      ...
      gst_msplitmux_sink_reset (splitmux);
...
static void
gst_msplitmux_sink_ensure_max_files (GstMSplitMuxSink * splitmux)
{
  guint max_files = splitmux->max_files;

  if (max_files == 0)
    return;

  while (g_queue_get_length (&splitmux->old_files) >= max_files) {
    gchar *filename;

    filename = g_queue_pop_head (&splitmux->old_files);
    g_remove (filename);
    g_free (filename);
  }
}
...
static void
split_now (GstMSplitMuxSink * splitmux)
{
  g_atomic_int_set (&(splitmux->do_split_next_gop), TRUE);
}

static void
split_after (GstMSplitMuxSink * splitmux)
{
  g_atomic_int_set (&(splitmux->split_requested), TRUE);
}

static void
split_at_running_time (GstMSplitMuxSink * splitmux, GstClockTime split_time)
{
