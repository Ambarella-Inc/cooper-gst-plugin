 ...
 */
/**
 * SECTION:element-mfilesink
 * @title: mfilesink
 * @see_also: #GstMfileSrc
 *
 * Write incoming data to a file in the local file system.
 *
 * ## Example launch line
 * |[
 * gst-launch-1.0 v4l2src num-buffers=1 ! jpegenc ! mfilesink location=capture1.jpeg
...
#include "gstmelements_private.h"
#include "gstmfilesink.h"
#include "internal.h"
...
#define GST_TYPE_MFILE_SINK_FILE_MODE (gst_mfile_sink_file_mode_get_type())
static GType
gst_mfile_sink_file_mode_get_type (void)
{
  static GType file_mode_type = 0;

  if (g_once_init_enter (&file_mode_type)) {
    static const GEnumValue file_mode[] = {
      {GST_MFILE_SINK_FILE_MODE_TRUNC, "Truncate file (mode wb)", "truncate"},
      {GST_MFILE_SINK_FILE_MODE_APPEND, "Append file (mode ab)", "output"},
      {GST_MFILE_SINK_FILE_MODE_OVERWRITE,
          "Overwrite file without truncating (mode rb+)", "overwrite"},
      {0, NULL, NULL}
    };

    GType new_file_mode_type =
        g_enum_register_static ("GstMfileSinkFileMode", file_mode);
...
#define GST_TYPE_MFILE_SINK_BUFFER_MODE (gst_mfile_sink_buffer_mode_get_type ())
static GType
gst_mfile_sink_buffer_mode_get_type (void)
{
  static GType buffer_mode_type = 0;
  static const GEnumValue buffer_mode[] = {
    {GST_MFILE_SINK_BUFFER_MODE_DEFAULT, "Default buffering", "default"},
    {GST_MFILE_SINK_BUFFER_MODE_FULL, "Fully buffered", "full"},
    {GST_MFILE_SINK_BUFFER_MODE_LINE, "Line buffered (deprecated, like full)",
        "line"},
    {GST_MFILE_SINK_BUFFER_MODE_UNBUFFERED, "Unbuffered", "unbuffered"},
    {0, NULL, NULL},
  };

  if (!buffer_mode_type) {
    buffer_mode_type =
        g_enum_register_static ("GstMfileSinkBufferMode", buffer_mode);
  }
  return buffer_mode_type;
}

GST_DEBUG_CATEGORY_STATIC (gst_mfile_sink_debug);
#define GST_CAT_DEFAULT gst_mfile_sink_debug

#define DEFAULT_LOCATION 	NULL
#define DEFAULT_BUFFER_MODE 	GST_MFILE_SINK_BUFFER_MODE_DEFAULT
#define DEFAULT_BUFFER_SIZE 	4194304 //(4 * 1024 * 1024)
#define DEFAULT_APPEND		FALSE
#define DEFAULT_O_SYNC		FALSE
#define DEFAULT_MAX_TRANSIENT_ERROR_TIMEOUT	0
#define DEFAULT_FILE_MODE      GST_MFILE_SINK_FILE_MODE_TRUNC
...
static void gst_mfile_sink_dispose (GObject * object);

static void gst_mfile_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_mfile_sink_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_mfile_sink_open_file (GstMfileSink * sink);
static void gst_mfile_sink_close_file (GstMfileSink * sink);

static gboolean gst_mfile_sink_start (GstBaseSink * sink);
static gboolean gst_mfile_sink_stop (GstBaseSink * sink);
static gboolean gst_mfile_sink_event (GstBaseSink * sink, GstEvent * event);
static GstFlowReturn gst_mfile_sink_render (GstBaseSink * sink,
    GstBuffer * buffer);
static GstFlowReturn gst_mfile_sink_render_list (GstBaseSink * sink,
    GstBufferList * list);
static gboolean gst_mfile_sink_unlock (GstBaseSink * sink);
static gboolean gst_mfile_sink_unlock_stop (GstBaseSink * sink);

static gboolean gst_mfile_sink_do_seek (GstMfileSink * filesink,
    guint64 new_offset);
static gboolean gst_mfile_sink_do_seekable (GstMfileSink * filesink,
    guint64 new_offset);

static gboolean gst_mfile_sink_get_current_offset (GstMfileSink * filesink,
    guint64 * p_pos);

static gboolean gst_mfile_sink_query (GstBaseSink * bsink, GstQuery * query);

static void gst_mfile_sink_uri_handler_init (gpointer g_iface,
    gpointer iface_data);

static GstFlowReturn gst_mfile_sink_flush_buffer (GstMfileSink * filesink);

#define _do_init \
  G_IMPLEMENT_INTERFACE (GST_TYPE_URI_HANDLER, gst_mfile_sink_uri_handler_init); \
  GST_DEBUG_CATEGORY_INIT (gst_mfile_sink_debug, "mfilesink", 0, "mfilesink element");
#define gst_mfile_sink_parent_class parent_class
/* cppcheck-suppress unknownMacro */
G_DEFINE_TYPE_WITH_CODE (GstMfileSink, gst_mfile_sink, GST_TYPE_BASE_SINK,
    _do_init);

static void
gst_mfile_sink_class_init (GstMfileSinkClass * klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  GstElementClass *gstelement_class = GST_ELEMENT_CLASS (klass);
  GstBaseSinkClass *gstbasesink_class = GST_BASE_SINK_CLASS (klass);

  gobject_class->dispose = gst_mfile_sink_dispose;

  gobject_class->set_property = gst_mfile_sink_set_property;
  gobject_class->get_property = gst_mfile_sink_get_property;
  ...
  g_object_class_install_property (gobject_class, PROP_BUFFER_MODE,
      g_param_spec_enum ("buffer-mode", "Buffering mode",
          "The buffering mode to use", GST_TYPE_MFILE_SINK_BUFFER_MODE,
  ...
  g_object_class_install_property (gobject_class, PROP_FILE_MODE,
      g_param_spec_enum ("file-mode", "File Mode",
          "Specify file mode used to open file", GST_TYPE_MFILE_SINK_FILE_MODE,
  ...
  gst_element_class_set_static_metadata (gstelement_class,
      "Mfile Sink",
      "Sink/Mfile", "Write stream to a file",
      "Thomas Vander Stichele <thomas at apestaart dot org>");
  gst_element_class_add_static_pad_template (gstelement_class, &sinktemplate);

  gstbasesink_class->start = GST_DEBUG_FUNCPTR (gst_mfile_sink_start);
  gstbasesink_class->stop = GST_DEBUG_FUNCPTR (gst_mfile_sink_stop);
  gstbasesink_class->query = GST_DEBUG_FUNCPTR (gst_mfile_sink_query);
  gstbasesink_class->render = GST_DEBUG_FUNCPTR (gst_mfile_sink_render);
  gstbasesink_class->render_list =
      GST_DEBUG_FUNCPTR (gst_mfile_sink_render_list);
  gstbasesink_class->event = GST_DEBUG_FUNCPTR (gst_mfile_sink_event);
  gstbasesink_class->unlock = GST_DEBUG_FUNCPTR (gst_mfile_sink_unlock);
  gstbasesink_class->unlock_stop =
      GST_DEBUG_FUNCPTR (gst_mfile_sink_unlock_stop);
  ...
  gst_type_mark_as_plugin_api (GST_TYPE_MFILE_SINK_BUFFER_MODE, 0);
  gst_type_mark_as_plugin_api (GST_TYPE_MFILE_SINK_FILE_MODE, 0);
}

static void
gst_mfile_sink_init (GstMfileSink * filesink)
{
  ...
  filesink->seekable = TRUE;

  gst_base_sink_set_sync (GST_BASE_SINK (filesink), FALSE);
}

static void
gst_mfile_sink_dispose (GObject * object)
{
  GstMfileSink *sink = GST_MFILE_SINK (object);
...
static gboolean
gst_mfile_sink_set_location (GstMfileSink * sink, const gchar * location,
    GError ** error)
{
  if (sink->file)
    goto was_open;
...
static void
gst_mfile_sink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstMfileSink *sink = GST_MFILE_SINK (object);
  ...
  switch (prop_id) {
    case PROP_LOCATION:
      gst_mfile_sink_set_location (sink, g_value_get_string (value), NULL);
      break;
...
static void
gst_mfile_sink_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstMfileSink *sink = GST_MFILE_SINK (object);
...
static gboolean __open_file (GstMfileSink * sink)
{
  /* open the file */
  ...
  if (sink->append || sink->file_mode == GST_MFILE_SINK_FILE_MODE_APPEND)
    sink->file = gst_fopen (sink->filename, "ab", sink->o_sync);
  else if (sink->file_mode == GST_MFILE_SINK_FILE_MODE_OVERWRITE)
  ...
  sink->seekable = gst_mfile_sink_do_seekable (sink, 0);
...
no_filename:
  {
    GST_ERROR_OBJECT (sink, "No file name specified for writing.");
    return FALSE;
  }

open_failed:
  {
    GST_ERROR_OBJECT (sink, "Could not open file %s for writing.", sink->filename);
    return FALSE;
  }
...
static gboolean
gst_mfile_sink_open_file (GstMfileSink * sink)
{
  /*if (__open_file(sink) == FALSE) {
     GST_ERROR_OBJECT (sink, "Render buffer failed\n");
    return FALSE;
  }*/
  ...
  if (sink->buffer_mode != GST_MFILE_SINK_BUFFER_MODE_UNBUFFERED) {
    if (sink->buffer_size == 0) {
      sink->buffer_size = DEFAULT_BUFFER_SIZE;
      g_object_notify (G_OBJECT (sink), "buffer-size");
    }

    if (sink->buffer_mode == GST_MFILE_SINK_BUFFER_MODE_FULL) {
...
static void
gst_mfile_sink_close_file (GstMfileSink * sink)
{
  if (sink->file) {
    if (gst_mfile_sink_flush_buffer (sink) != GST_FLOW_OK)
      GST_ERROR_OBJECT (sink, "Error closing file %s.", sink->filename);
...
static gboolean
gst_mfile_sink_query (GstBaseSink * bsink, GstQuery * query)
{
  gboolean res;
  GstMfileSink *self;
  GstFormat format;

  self = GST_MFILE_SINK (bsink);
...
static gboolean
gst_mfile_sink_do_seek (GstMfileSink * filesink, guint64 new_offset)
{
  GST_DEBUG_OBJECT (filesink, "Seeking to offset %" G_GUINT64_FORMAT
      " using " __GST_STDIO_SEEK_FUNCTION, new_offset);

  if (gst_mfile_sink_flush_buffer (filesink) != GST_FLOW_OK)
  ...
  gst_mfile_sink_get_current_offset (filesink, &filesink->current_pos);
...
static gboolean
gst_mfile_sink_do_seekable (GstMfileSink * filesink, guint64 new_offset)
{
  GST_DEBUG_OBJECT (filesink, "Seeking to offset %" G_GUINT64_FORMAT
      " using " __GST_STDIO_SEEK_FUNCTION, new_offset);
...
static gboolean
gst_mfile_sink_event (GstBaseSink * sink, GstEvent * event)
{
  GstEventType type;
  GstMfileSink *filesink;

  filesink = GST_MFILE_SINK (sink);
  ...
  switch (type) {
    case GST_EVENT_SEGMENT:
    {
      ...
      if (segment->format == GST_FORMAT_BYTES) {
        ...
        if (filesink->current_pos + filesink->current_buffer_size !=
          ...
          if (!gst_mfile_sink_do_seek (filesink, (guint64) segment->start))
            goto seek_failed;
        } else {
    ...
    case GST_EVENT_FLUSH_STOP:
      if (filesink->current_pos != 0 && filesink->seekable) {
        gst_mfile_sink_do_seek (filesink, 0);
...
seek_failed:
  {
    GST_ERROR_OBJECT (filesink, "Error while seeking in file %s.", filesink->filename);
    gst_event_unref (event);
    return FALSE;
  }
flush_buffer_failed:
  {
    GST_ERROR_OBJECT (filesink, "Error while writing to file %s.", filesink->filename);
    gst_event_unref (event);
    return FALSE;
  }
truncate_failed:
...
static gboolean
gst_mfile_sink_get_current_offset (GstMfileSink * filesink, guint64 * p_pos)
{
  off_t ret = -1;
...
gst_mfile_sink_render_list_internal (GstMfileSink * sink,
    GstBufferList * buffer_list)
{
  ...
  if (sink->file == NULL) {
    if (__open_file(sink) == FALSE) {
       GST_ERROR_OBJECT (sink, "Render list internal failed\n");
  ...
  for (;;) {
    ...
    flow =
        gst_m_writev_buffer_list (GST_OBJECT_CAST (sink), fileno (sink->file),
...
static GstFlowReturn
gst_mfile_sink_flush_buffer (GstMfileSink * filesink)
{
  ...
  if (filesink->buffer && filesink->current_buffer_size) {
    ...
    if (filesink->file == NULL) {
      if (__open_file(filesink) == FALSE) {
         GST_ERROR_OBJECT (filesink, "Flushing out buffer failed\n");
        return GST_FLOW_ERROR;
      }
    }
    ...
    for (;;) {
      ...
      flow_ret =
          gst_m_writev_mem (GST_OBJECT_CAST (filesink), fileno (filesink->file),
    ...
    if (length > 0) {
      flow_ret =
          gst_mfile_sink_render_list_internal (filesink, filesink->buffer_list);
...
static GstFlowReturn
render_buffer (GstMfileSink * filesink, GstBuffer * buffer)
{
  GstFlowReturn flow;
  guint64 bytes_written = 0;
  guint64 skip = 0;
  ...
  if (filesink->file == NULL) {
    if (__open_file(filesink) == FALSE) {
       GST_ERROR_OBJECT (filesink, "Render buffer failed\n");
      return GST_FLOW_ERROR;
    }
  }
  ...
  for (;;) {
    flow =
        gst_m_writev_buffer (GST_OBJECT_CAST (filesink),
...
static GstFlowReturn
gst_mfile_sink_render_list (GstBaseSink * bsink, GstBufferList * buffer_list)
{
  GstFlowReturn flow;
  GstMfileSink *sink;
  guint i, num_buffers;
  ...
  sink = GST_MFILE_SINK_CAST (bsink);
  ...
  if (sync_after || (!sink->buffer && !sink->buffer_list)) {
    flow = gst_mfile_sink_flush_buffer (sink);
    if (flow == GST_FLOW_OK)
      flow = gst_mfile_sink_render_list_internal (sink, buffer_list);
  } else {
    guint size = 0;
  ...
  if (flow == GST_FLOW_OK && sync_after) {
    ...
    if (fsync_ret) {
      GST_ERROR_OBJECT (sink, "Error (%s) while writing to file %s.", g_strerror (errno), sink->filename);
      flow = GST_FLOW_ERROR;
    }
  }
...
static GstFlowReturn
gst_mfile_sink_render (GstBaseSink * sink, GstBuffer * buffer)
{
  GstMfileSink *filesink;
  GstFlowReturn flow;
  guint8 n_mem;
  ...
  filesink = GST_MFILE_SINK_CAST (sink);
  ...
  if (n_mem > 0 && (sync_after || (!filesink->buffer
    ...
    flow = gst_mfile_sink_flush_buffer (filesink);
    if (flow == GST_FLOW_OK) {
  ...
  if (flow == GST_FLOW_OK && sync_after) {
    ...
    if (fsync_ret) {
      GST_ERROR_OBJECT (filesink, "Error (%s) while writing to file %s.", g_strerror (errno), filesink->filename);
      flow = GST_FLOW_ERROR;
    }
  }
...
static gboolean
gst_mfile_sink_start (GstBaseSink * basesink)
{
  GstMfileSink *filesink;

  filesink = GST_MFILE_SINK_CAST (basesink);

  g_atomic_int_set (&filesink->flushing, FALSE);
  return gst_mfile_sink_open_file (filesink);
}

static gboolean
gst_mfile_sink_stop (GstBaseSink * basesink)
{
  GstMfileSink *filesink;

  filesink = GST_MFILE_SINK_CAST (basesink);

  gst_mfile_sink_close_file (filesink);
  return TRUE;
}

static gboolean
gst_mfile_sink_unlock (GstBaseSink * basesink)
{
  GstMfileSink *filesink;
...
static gboolean
gst_mfile_sink_unlock_stop (GstBaseSink * basesink)
{
  GstMfileSink *filesink;
...
static GstURIType
gst_mfile_sink_uri_get_type (GType type)
{
  DUNUSED(type);
  return GST_URI_SINK;
}
...
gst_mfile_sink_uri_get_protocols (GType type)
{
  DUNUSED(type);
...
static gchar *
gst_mfile_sink_uri_get_uri (GstURIHandler * handler)
{
  GstMfileSink *sink = GST_MFILE_SINK (handler);
...
static gboolean
gst_mfile_sink_uri_set_uri (GstURIHandler * handler, const gchar * uri,
    GError ** error)
{
  gchar *location;
  gboolean ret;
  ...
  if (g_str_has_prefix (uri, "file://localhost/")) {
    ...
    gst_mfile_sink_set_location (sink, NULL, NULL);
    return TRUE;
  } else {
  ...
  ret = gst_mfile_sink_set_location (sink, location, error);
  g_free (location);
...
static void
gst_mfile_sink_uri_handler_init (gpointer g_iface, gpointer iface_data)
{
  DUNUSED(iface_data);
  GstURIHandlerInterface *iface = (GstURIHandlerInterface *) g_iface;

  iface->get_type = gst_mfile_sink_uri_get_type;
  iface->get_protocols = gst_mfile_sink_uri_get_protocols;
  iface->get_uri = gst_mfile_sink_uri_get_uri;
  iface->set_uri = gst_mfile_sink_uri_set_uri;
}
