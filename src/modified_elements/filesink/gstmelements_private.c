...
gchar *
gst_m_buffer_get_flags_string (GstBuffer * buffer)
{
  ...
  for (i = BUFFER_FLAG_SHIFT; i < G_N_ELEMENTS (flag_idx); i++) {
    if (GST_MINI_OBJECT_CAST (buffer)->flags & (1 << i)) {
      strncpy (end, flag_strings + flag_idx[i], max_bytes);
      end += strlen (end);
      end[0] = ' ';
      end[1] = '\0';
      end++;
...
gchar *
gst_m_buffer_get_meta_string (GstBuffer * buffer)
{
  gpointer state = NULL;
  GstMeta *meta;
  GString *s = NULL;
  ...
  /* now write it all out! */
  {
    ...
    do {
      ...
      if (errno == EAGAIN || errno == EWOULDBLOCK || ret == 0) {
        ...
        g_assert (current_position != (guint64) -1);
        g_usleep (10000);
        ...
        if (ret < 0 || ret != (gssize) current_position) {
...
GstFlowReturn
gst_m_writev_buffer (GstObject * sink, gint fd, GstPoll * fdset,
    GstBuffer * buffer,
...
GstFlowReturn
gst_m_writev_mem (GstObject * sink, gint fd, GstPoll * fdset,
...
GstFlowReturn
gst_m_writev_buffer_list (GstObject * sink, gint fd, GstPoll * fdset,
