...
static GstCaps *
gst_malsa_detect_rates (GstObject * obj, snd_pcm_hw_params_t * hw_params,
    GstCaps * in_caps)
{
  GstCaps *caps;
  guint min, max;
...
gst_malsa_get_pcm_format (GstAudioFormat fmt)
{
  switch (fmt) {
    ...
    int endianness)
{
  ...
  pcm_format = gst_malsa_get_pcm_format (format);
...
static GstCaps *
gst_malsa_detect_formats (GstObject * obj, snd_pcm_hw_params_t * hw_params,
...
#define GST_MALSA_MAX_CHANNELS 8
...
static GstCaps *
gst_malsa_detect_channels (GstObject * obj, snd_pcm_hw_params_t * hw_params,
    GstCaps * in_caps)
{
  GstCaps *caps;
  guint min, max;
  ...
  if (min_chans < 0) {
    min_chans = 1;
    max_chans = GST_MALSA_MAX_CHANNELS;
  } else if (max_chans < 0) {
    max_chans = GST_MALSA_MAX_CHANNELS;
  }
  ...
  if (min_chans > GST_MALSA_MAX_CHANNELS) {
    ...
    max_chans = MIN (GST_MALSA_MAX_CHANNELS, max_chans);
  }
...
snd_pcm_t *
gst_malsa_open_iec958_pcm (GstObject * obj, gchar * device)
{
...
/*
 * gst_malsa_probe_supported_formats:
 *
...
GstCaps *
gst_malsa_probe_supported_formats (GstObject * obj, gchar * device,
    snd_pcm_t * handle, const GstCaps * template_caps)
{
  ...
  caps = gst_malsa_detect_formats (obj, hw_params,
  ...
  if (!(caps = gst_malsa_detect_rates (obj, hw_params, caps)))
    goto subroutine_error;

  if (!(caps = gst_malsa_detect_channels (obj, hw_params, caps)))
    goto subroutine_error;
  ...
  if (stream_type == SND_PCM_STREAM_PLAYBACK) {
    snd_pcm_t *pcm = gst_malsa_open_iec958_pcm (obj, device);
...
static gchar *
gst_malsa_find_device_name_no_handle (GstObject * obj, const gchar * devcard,
...
gchar *
gst_malsa_find_card_name (GstObject * obj, const gchar * devcard,
    snd_pcm_stream_t stream)
{
  return gst_malsa_find_device_name_no_handle (obj, devcard, -1, stream);
}

gchar *
gst_malsa_find_device_name (GstObject * obj, const gchar * device,
    snd_pcm_t * handle, snd_pcm_stream_t stream)
{
  ...
  if (device != NULL) {
    ...
    if ((comma = strchr (dev, ','))) {
      *comma = '\0';
      devnum = atoi (comma + 1);
      ret = gst_malsa_find_device_name_no_handle (obj, dev, devnum, stream);
    }
    g_free (dev);
  }
...
const GstAudioChannelPosition malsa_position[][8] = {
  {
...
gboolean
malsa_chmap_to_channel_positions (const snd_pcm_chmap_t * chmap,
...
void
malsa_detect_channels_mapping (GstObject * obj, snd_pcm_t * handle,
    GstAudioRingBufferSpec * spec, guint channels, GstAudioRingBuffer * buf)
{
  ...
  if (malsa_chmap_to_channel_positions (chmap, pos)) {
