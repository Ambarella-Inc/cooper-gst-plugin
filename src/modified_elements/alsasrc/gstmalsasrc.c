...
GST_DEBUG_CATEGORY_STATIC (gst_malsasrc_debug);
#define GST_CAT_DEFAULT gst_malsasrc_debug
...
#define gst_malsasrc_parent_class parent_class


G_DEFINE_TYPE_WITH_CODE (GstMalsaSrc, gst_malsasrc, GST_TYPE_AUDIO_SRC,
    GST_DEBUG_CATEGORY_INIT (gst_malsasrc_debug, "malsasrc", 0,
        "malsasrc"));


static void gst_malsasrc_finalize (GObject * object);
static void gst_malsasrc_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_malsasrc_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);
static GstStateChangeReturn gst_malsasrc_change_state (GstElement * element,
    GstStateChange transition);
static GstCaps *gst_malsasrc_getcaps (GstBaseSrc * bsrc, GstCaps * filter);

static gboolean gst_malsasrc_open (GstAudioSrc * asrc);
static gboolean gst_malsasrc_prepare (GstAudioSrc * asrc,
    GstAudioRingBufferSpec * spec);
static gboolean gst_malsasrc_unprepare (GstAudioSrc * asrc);
static gboolean gst_malsasrc_close (GstAudioSrc * asrc);
static guint gst_malsasrc_read
    (GstAudioSrc * asrc, gpointer data, guint length, GstClockTime * timestamp);
static guint gst_malsasrc_delay (GstAudioSrc * asrc);
static void gst_malsasrc_reset (GstAudioSrc * asrc);
...
# define MALSA_SRC_FACTORY_ENDIANNESS   "LITTLE_ENDIAN, BIG_ENDIAN"
#else
# define MALSA_SRC_FACTORY_ENDIANNESS   "BIG_ENDIAN, LITTLE_ENDIAN"
#endif

static GstStaticPadTemplate malsasrc_src_factory =
...
static GstClockTime get_time(GstMalsaSrc * thiz)
{
  GstClockTimeDiff gst_clok_time;
  if (!thiz->is_clock_started) {
    gst_clok_time = gst_clock_get_time(GST_ELEMENT_CLOCK(thiz)) - GST_ELEMENT_CAST (thiz)->base_time;
    start_clock(thiz->clock, gst_clok_time);
    thiz->is_clock_started = 1;
  } else {
    gst_clok_time = get_cur_clock(thiz->clock);
  }

  return gst_clok_time;
}

static void
gst_malsasrc_finalize (GObject * object)
{
  GstMalsaSrc *src = GST_MALSA_SRC (object);

  g_free (src->device);
  g_mutex_clear (&src->alsa_lock);

  if (src->clock) {
    destroy_clock(src->clock);
    src->clock = NULL;
  }
...
static void
gst_malsasrc_class_init (GstMalsaSrcClass * klass)
{
  ...
  gobject_class->finalize = gst_malsasrc_finalize;
  gobject_class->get_property = gst_malsasrc_get_property;
  gobject_class->set_property = gst_malsasrc_set_property;
  ...
  gst_element_class_add_static_pad_template (gstelement_class,
      &malsasrc_src_factory);

  gstbasesrc_class->get_caps = GST_DEBUG_FUNCPTR (gst_malsasrc_getcaps);

  gstaudiosrc_class->open = GST_DEBUG_FUNCPTR (gst_malsasrc_open);
  gstaudiosrc_class->prepare = GST_DEBUG_FUNCPTR (gst_malsasrc_prepare);
  gstaudiosrc_class->unprepare = GST_DEBUG_FUNCPTR (gst_malsasrc_unprepare);
  gstaudiosrc_class->close = GST_DEBUG_FUNCPTR (gst_malsasrc_close);
  gstaudiosrc_class->read = GST_DEBUG_FUNCPTR (gst_malsasrc_read);
  gstaudiosrc_class->delay = GST_DEBUG_FUNCPTR (gst_malsasrc_delay);
  gstaudiosrc_class->reset = GST_DEBUG_FUNCPTR (gst_malsasrc_reset);
  gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_malsasrc_change_state);
  ...
  g_object_class_install_property (gobject_class, PROP_USE_HW_TIMER,
      g_param_spec_boolean ("use-hw-timer", "Use hw timer",
          "Use hw timer timestamps",
          TRUE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void
gst_malsasrc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstMalsaSrc *src;

  src = GST_MALSA_SRC (object);
  ...
  switch (prop_id) {
    ...
    case PROP_USE_DRIVER_TIMESTAMP:
      ...
      break;
    case PROP_USE_HW_TIMER:
      GST_OBJECT_LOCK (src);
      src->use_hw_timer = g_value_get_boolean (value);
      GST_OBJECT_UNLOCK (src);
      break;
    default:
...
static void
gst_malsasrc_get_property (GObject * object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_DEVICE_NAME:
      g_value_take_string (value,
          gst_malsa_find_device_name (GST_OBJECT_CAST (src),
              src->device, src->handle, SND_PCM_STREAM_CAPTURE));
      break;
    case PROP_CARD_NAME:
      g_value_take_string (value,
          gst_malsa_find_card_name (GST_OBJECT_CAST (src),
    ...
    case PROP_USE_HW_TIMER:
      GST_OBJECT_LOCK (src);
      g_value_set_boolean (value, src->use_hw_timer);
      GST_OBJECT_UNLOCK (src);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
gst_malsasrc_change_state (GstElement * element, GstStateChange transition)
{
  GstStateChangeReturn ret = GST_STATE_CHANGE_SUCCESS;
  GstMalsaSrc *alsa = GST_MALSA_SRC (element);
  GstClock *clk;
...
static void
gst_malsasrc_init (GstMalsaSrc * alsasrc)
{
  ...
  alsasrc->gst_clok_time = 0;
  alsasrc->is_clock_started = 0;
  alsasrc->clock = create_clock ("amba_hwtimer",
      gst_amba_hwtimer_get_outfreq(), GST_SECOND);
  if (!alsasrc->clock) {
    DPRINT_ERROR("clock create failed\n");
    alsasrc->is_clock_setup = 0;
  } else {
    alsasrc->is_clock_setup = 1;
  }
...
static GstCaps *
gst_malsasrc_getcaps (GstBaseSrc * bsrc, GstCaps * filter)
{
  ...
  caps = gst_malsa_probe_supported_formats (GST_OBJECT (src),
...
set_hwparams (GstMalsaSrc * alsa)
{
   ...
   */
  if (alsa->period_time != (guint) -1 && alsa->buffer_time != (guint) -1) {
...
no_channels:
  {
    ...
    if ((alsa->channels) == 1)
      msg = g_strdup ("Could not open device for recording in mono mode.");
    if ((alsa->channels) == 2)
      msg = g_strdup ("Could not open device for recording in stereo mode.");
    if ((alsa->channels) > 2)
      msg =
          g_strdup_printf (
...
static gboolean
malsasrc_parse_spec (GstMalsaSrc * alsa, GstAudioRingBufferSpec * spec)
{
  switch (spec->type) {
  ...
  if (spec->type == GST_AUDIO_RING_BUFFER_FORMAT_TYPE_RAW && alsa->channels < 9)
    gst_audio_ring_buffer_set_channel_positions (GST_AUDIO_BASE_SRC
        (alsa)->ringbuffer, malsa_position[alsa->channels - 1]);
...
static gboolean
gst_malsasrc_open (GstAudioSrc * asrc)
{
  GstMalsaSrc *alsa;
  gint err;

  alsa = GST_MALSA_SRC (asrc);
...
static gboolean
gst_malsasrc_prepare (GstAudioSrc * asrc, GstAudioRingBufferSpec * spec)
{
  GstMalsaSrc *alsa;
  gint err;
  ...
  if (!malsasrc_parse_spec (alsa, spec))
    goto spec_parse;
  ...
  malsa_detect_channels_mapping (GST_OBJECT (alsa), alsa->handle, spec,
...
static gboolean
gst_malsasrc_unprepare (GstAudioSrc * asrc)
{
  GstMalsaSrc *alsa;
...
static gboolean
gst_malsasrc_close (GstAudioSrc * asrc)
{
  GstMalsaSrc *alsa = GST_MALSA_SRC (asrc);
 ...
 */
static gint
xrun_recovery (GstMalsaSrc * alsa, snd_pcm_t * handle, gint err)
{
...
static GstClockTime
gst_malsasrc_get_timestamp (GstMalsaSrc * asrc)
{
...
static guint
gst_malsasrc_read (GstAudioSrc * asrc, gpointer data, guint length,
    GstClockTime * timestamp)
{
  GstMalsaSrc *alsa;
  gint err;
  ...
  GST_MALSA_SRC_UNLOCK (asrc);

  /* if driver timestamps are enabled we need to return this here */

  if (alsa->use_hw_timer && timestamp) {
    *timestamp = get_time(alsa);
  } else if (alsa->driver_timestamps && timestamp)
    *timestamp = gst_malsasrc_get_timestamp (alsa);

  if (alsa->dump_num > 0) {
    DPRINT_NOTICE ("base time: %ld (ns), ALSA time : %ld (ns), timestamp : %" GST_TIME_FORMAT
        ", interval: %ld (ms)\n", GST_ELEMENT_CAST (asrc)->base_time, *timestamp, GST_TIME_ARGS (*timestamp),
        GST_TIME_AS_MSECONDS (*timestamp - GST_ELEMENT_CAST (asrc)->base_time));
    alsa->dump_num--;
  }
...
static guint
gst_malsasrc_delay (GstAudioSrc * asrc)
{
  GstMalsaSrc *alsa;
  ...
  return CLAMP (delay, 0, (snd_pcm_sframes_t) alsa->buffer_size);
}

static void
gst_malsasrc_reset (GstAudioSrc * asrc)
{
  GstMalsaSrc *alsa;
  gint err;
