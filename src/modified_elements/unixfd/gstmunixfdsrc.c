...
GST_DEBUG_CATEGORY (munixfdsrc_debug);
#define GST_CAT_DEFAULT (munixfdsrc_debug)
...
#define GST_TYPE_MUNIX_FD_SRC gst_munix_fd_src_get_type()
G_DECLARE_FINAL_TYPE (GstMUnixFdSrc, gst_munix_fd_src, GST, MUNIX_FD_SRC,
    GstPushSrc);

struct _GstMUnixFdSrc
{
  GstPushSrc parent;
...
G_DEFINE_TYPE (GstMUnixFdSrc, gst_munix_fd_src, GST_TYPE_PUSH_SRC);
GST_ELEMENT_REGISTER_DEFINE (munixfdsrc, "munixfdsrc", GST_RANK_NONE,
    GST_TYPE_MUNIX_FD_SRC);
...
static void
release_buffer (GstMUnixFdSrc * self, guint64 id)
{
...
static void
memory_weak_ref_cb (GstMUnixFdSrc * self, GstMemory * mem)
{
...
static void
gst_munix_fd_src_init (GstMUnixFdSrc * self)
{
  g_return_if_fail (GST_IS_MUNIX_FD_SRC (self));
...
static void
gst_munix_fd_src_finalize (GObject * object)
{
  GstMUnixFdSrc *self = GST_MUNIX_FD_SRC (object);
  ...
  G_OBJECT_CLASS (gst_munix_fd_src_parent_class)->finalize (object);
}

static void
gst_munix_fd_src_set_property (GObject * object, guint prop_id,
...
static void
gst_munix_fd_src_get_property (GObject * object, guint prop_id,
...
static gboolean
gst_munix_fd_src_start (GstBaseSrc * bsrc)
{
  GstMUnixFdSrc *self = (GstMUnixFdSrc *) bsrc;
...
static gboolean
gst_munix_fd_src_stop (GstBaseSrc * bsrc)
{
  GstMUnixFdSrc *self = (GstMUnixFdSrc *) bsrc;
...
static gboolean
gst_munix_fd_src_unlock (GstBaseSrc * bsrc)
{
  GstMUnixFdSrc *self = GST_MUNIX_FD_SRC (bsrc);
  g_cancellable_cancel (self->cancellable);
  return TRUE;
}

static gboolean
gst_munix_fd_src_unlock_stop (GstBaseSrc * bsrc)
{
  GstMUnixFdSrc *self = GST_MUNIX_FD_SRC (bsrc);
...
gst_munix_fd_src_create (GstPushSrc * psrc, GstBuffer ** outbuf)
{
  GstMUnixFdSrc *self = GST_MUNIX_FD_SRC (psrc);
  CommandType command;
...
static gboolean
gst_munix_fd_src_set_clock (GstElement * element, GstClock * clock)
{
  GstMUnixFdSrc *self = (GstMUnixFdSrc *) element;
  ...
  return GST_ELEMENT_CLASS (gst_munix_fd_src_parent_class)->set_clock (element,
      clock);
}

static void
gst_munix_fd_src_class_init (GstMUnixFdSrcClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (munixfdsrc_debug, "munixfdsrc", 0,
      "Unix file descriptor source");
  gst_element_class_set_static_metadata (gstelement_class,
      "MUnix file descriptor source", "Src", "Modified Unix file descriptor source",
      "Xavier Claessens <xavier.claessens@collabora.com>");
  gst_element_class_add_static_pad_template (gstelement_class, &srctemplate);

  gobject_class->finalize = gst_munix_fd_src_finalize;
  gobject_class->set_property = gst_munix_fd_src_set_property;
  gobject_class->get_property = gst_munix_fd_src_get_property;

  gstelement_class->set_clock = GST_DEBUG_FUNCPTR (gst_munix_fd_src_set_clock);

  gstbasesrc_class->start = GST_DEBUG_FUNCPTR (gst_munix_fd_src_start);
  gstbasesrc_class->stop = GST_DEBUG_FUNCPTR (gst_munix_fd_src_stop);
  gstbasesrc_class->unlock = GST_DEBUG_FUNCPTR (gst_munix_fd_src_unlock);
  gstbasesrc_class->unlock_stop =
      GST_DEBUG_FUNCPTR (gst_munix_fd_src_unlock_stop);

  gstpushsrc_class->create = gst_munix_fd_src_create;
