...
GST_DEBUG_CATEGORY (munixfdsink_debug);
#define GST_CAT_DEFAULT (munixfdsink_debug)
...
#define GST_TYPE_MUNIX_FD_SINK gst_munix_fd_sink_get_type()
G_DECLARE_FINAL_TYPE (GstMUnixFdSink, gst_munix_fd_sink, GST, MUNIX_FD_SINK,
    GstBaseSink);
...
G_DEFINE_TYPE (GstMUnixFdSink, gst_munix_fd_sink, GST_TYPE_BASE_SINK);
GST_ELEMENT_REGISTER_DEFINE (munixfdsink, "munixfdsink", GST_RANK_NONE,
    GST_TYPE_MUNIX_FD_SINK);
...
static void
gst_munix_fd_sink_init (GstMUnixFdSink * self)
{
  g_return_if_fail (GST_IS_MUNIX_FD_SINK (self));
...
static void
gst_munix_fd_sink_finalize (GObject * object)
{
  GstMUnixFdSink *self = GST_MUNIX_FD_SINK (object);
  ...
  G_OBJECT_CLASS (gst_munix_fd_sink_parent_class)->finalize (object);
}

static void
gst_munix_fd_sink_set_property (GObject * object, guint prop_id,
...
static void
gst_munix_fd_sink_get_property (GObject * object, guint prop_id,
...
static gboolean
gst_munix_fd_sink_start (GstBaseSink * bsink)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) bsink;
  ...
  self->thread = g_thread_new ("munixfdsink", thread_cb, self);
...
static gboolean
gst_munix_fd_sink_stop (GstBaseSink * bsink)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) bsink;
...
static void
send_command_to_all (GstMUnixFdSink * self, CommandType type, GUnixFDList * fds,
...
gst_munix_fd_sink_render (GstBaseSink * bsink, GstBuffer * buffer)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) bsink;
...
static gboolean
gst_munix_fd_sink_unlock (GstBaseSink * bsink)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) bsink;
...
static gboolean
gst_munix_fd_sink_unlock_stop (GstBaseSink * bsink)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) bsink;
...
static gboolean
gst_munix_fd_sink_event (GstBaseSink * bsink, GstEvent * event)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) bsink;
  ...
  return GST_BASE_SINK_CLASS (gst_munix_fd_sink_parent_class)->event (bsink,
      event);
}

static gboolean
gst_munix_fd_sink_propose_allocation (GstBaseSink * bsink, GstQuery * query)
{
...
static gboolean
gst_munix_fd_sink_set_clock (GstElement * element, GstClock * clock)
{
  GstMUnixFdSink *self = (GstMUnixFdSink *) element;
  ...
  return GST_ELEMENT_CLASS (gst_munix_fd_sink_parent_class)->set_clock (element,
      clock);
}

static void
gst_munix_fd_sink_class_init (GstMUnixFdSinkClass * klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstElementClass *gstelement_class = (GstElementClass *) klass;
  GstBaseSinkClass *gstbasesink_class = (GstBaseSinkClass *) klass;

  GST_DEBUG_CATEGORY_INIT (munixfdsink_debug, "munixfdsink", 0,
      "Unix file descriptor sink");
  gst_element_class_set_static_metadata (gstelement_class,
      "MUnix file descriptor sink", "Sink", "Modified Unix file descriptor sink",
      "Xavier Claessens <xavier.claessens@collabora.com>");
  gst_element_class_add_static_pad_template (gstelement_class, &sinktemplate);
  //gst_shm_allocator_init_once ();
  gst_amba_cavalry_allocator_init_once ();

  gobject_class->finalize = gst_munix_fd_sink_finalize;
  gobject_class->set_property = gst_munix_fd_sink_set_property;
  gobject_class->get_property = gst_munix_fd_sink_get_property;

  gstelement_class->set_clock = GST_DEBUG_FUNCPTR (gst_munix_fd_sink_set_clock);

  gstbasesink_class->start = GST_DEBUG_FUNCPTR (gst_munix_fd_sink_start);
  gstbasesink_class->stop = GST_DEBUG_FUNCPTR (gst_munix_fd_sink_stop);
  gstbasesink_class->render = GST_DEBUG_FUNCPTR (gst_munix_fd_sink_render);
  gstbasesink_class->event = GST_DEBUG_FUNCPTR (gst_munix_fd_sink_event);
  gstbasesink_class->propose_allocation =
      GST_DEBUG_FUNCPTR (gst_munix_fd_sink_propose_allocation);
  gstbasesink_class->unlock = GST_DEBUG_FUNCPTR (gst_munix_fd_sink_unlock);
  gstbasesink_class->unlock_stop =
      GST_DEBUG_FUNCPTR (gst_munix_fd_sink_unlock_stop);
  ...
  /**
   * GstMUnixFdSink:wait-for-connection:
   *
