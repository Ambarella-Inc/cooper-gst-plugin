...
/**
 * SECTION: element-amba_camsrc
 * @title: amba_camsrc
 *
 * amba_camsrc can be used to capture video frames from Amba device.
 *
 */
...
GST_DEBUG_CATEGORY_STATIC (gst_amba_camsrc_debug);
#define GST_CAT_DEFAULT gst_amba_camsrc_debug
...
#define gst_amba_camsrc_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE (GstAmbaCamsrc, gst_amba_camsrc, GST_TYPE_PUSH_SRC,
    GST_DEBUG_CATEGORY_INIT (gst_amba_camsrc_debug, "amba_camsrc", 0,
        "amba_camsrc") );

static GstFlowReturn gst_amba_camsrc_create (GstPushSrc *psrc,
    GstBuffer **outbuf);
static void gst_amba_camsrc_finalize (GObject *gobject);
static void gst_amba_camsrc_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec);
static void gst_amba_camsrc_get_property (GObject *object, guint prop_id,
    GValue *value, GParamSpec *pspec);

static GstStateChangeReturn
gst_amba_camsrc_change_state (GstElement *element, GstStateChange transition);

static GstCaps * gst_amba_camsrc_get_caps (GstBaseSrc * bsrc, GstCaps * caps);
static gboolean gst_amba_camsrc_start (GstBaseSrc * bsrc);

static GstClock *
gst_amba_camsrc_provide_clock (GstElement * element);


static gboolean
gst_amba_camsrc_send_event (GstElement * element, GstEvent * event);

/* initialize the amba_camsrc class */
static void gst_amba_camsrc_class_init (GstAmbaCamsrcClass *klass)
{
  ...
  gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_amba_camsrc_change_state);
  gstelement_class->provide_clock = GST_DEBUG_FUNCPTR (gst_amba_camsrc_provide_clock);
  gstelement_class->send_event = GST_DEBUG_FUNCPTR (gst_amba_camsrc_send_event);

  gobject_class->finalize = gst_amba_camsrc_finalize;
  gobject_class->set_property = gst_amba_camsrc_set_property;
  gobject_class->get_property = gst_amba_camsrc_get_property;
  ...
  gstbasesrc_class->start = GST_DEBUG_FUNCPTR (gst_amba_camsrc_start);
  gstbasesrc_class->get_caps = GST_DEBUG_FUNCPTR (gst_amba_camsrc_get_caps);
  gstpushsrc_class->create = GST_DEBUG_FUNCPTR (gst_amba_camsrc_create);
}

static void gst_amba_camsrc_init (GstAmbaCamsrc *thiz)
{
  // mem init
...
static void gst_amba_camsrc_finalize (GObject *gobject)
{
  GstAmbaCamsrc * thiz = GST_AMBA_CAMSRC (gobject);
...
gst_amba_camsrc_provide_clock (GstElement * element)
{
  GstAmbaCamsrc * src = GST_AMBA_CAMSRC (element);
...
static void
gst_amba_camsrc_set_provide_clock (GstAmbaCamsrc * src, gboolean provide)
{
  GST_OBJECT_LOCK (src);
  if (provide)
...
static gboolean
gst_amba_camsrc_get_provide_clock (GstAmbaCamsrc * src)
{
  gboolean result;
...
static void gst_amba_camsrc_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  GstAmbaCamsrc * thiz = GST_AMBA_CAMSRC (object);
  ...
  switch (prop_id) {
    ...
    case PROP_GDMA: {
      thiz->gdma_copy_enable = g_value_get_uchar (value);
    }
    break;
    ...
    case PROP_PROVIDE_CLOCK:
      gst_amba_camsrc_set_provide_clock (thiz, g_value_get_boolean (value));
    ...
    GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PROVIDE_CLOCK:
      g_value_set_boolean (value, gst_amba_camsrc_get_provide_clock (thiz));
      break;
    case PROP_CAPTURE_TYPE:
...
gst_amba_camsrc_create (GstPushSrc * psrc,
    GstBuffer ** outbuf)
{
  GstAmbaCamsrc * thiz = GST_AMBA_CAMSRC (psrc);
  ...
  int ret = 0;
  gsize me0_size = 0, me1_size = 0, addr_offset = 0;
  ...
  while (1) {
    ...
    if (thiz->gdma_copy_enable) {
      offset[0] = 0;
      offset[1] = yuv_buf.yuv_ctx[cur_id].pitch * yuv_buf.yuv_ctx[cur_id].height;

      mem0 = gst_allocator_alloc (NULL, offset[1] * 3 / 2 + me0_size + me1_size, NULL);
      if (!mem0) {
        GST_ERROR_OBJECT(thiz, "Failed to allocate memory for GDMA copy");
        if (thiz->canvas_map_thru_dmabuf) {
          if (yuv_buf.yuv_ctx[cur_id].yuv_dma_buf_fd >= 0) {
            close(yuv_buf.yuv_ctx[cur_id].yuv_dma_buf_fd);
            yuv_buf.yuv_ctx[cur_id].yuv_dma_buf_fd = -1;
      ...
      memset(&info, 0x0, sizeof(GstMapInfo));
      gst_memory_map(mem0, &info, GST_MAP_WRITE);
      memset(info.data, 0, info.size);
      memcpy (info.data, yuv_buf.gdma_ctx[cur_id].gdma_buf + yuv_buf.gdma_ctx[cur_id].y_addr_offset, offset[1]);
      addr_offset += offset[1];
      memcpy (info.data + addr_offset, yuv_buf.gdma_ctx[cur_id].gdma_buf + yuv_buf.gdma_ctx[cur_id].uv_addr_offset, offset[1] / 2);
      addr_offset += offset[1] / 2;

      meta->private_data_count = 0;
      if (thiz->capture_me0) {
        memcpy (info.data + addr_offset, yuv_buf.gdma_ctx[cur_id].gdma_buf + yuv_buf.gdma_ctx[cur_id].me0_addr_offset, me0_size);
        addr_offset += me0_size;
      ...
      if (thiz->capture_me1) {
        memcpy (info.data + addr_offset, yuv_buf.gdma_ctx[cur_id].gdma_buf + yuv_buf.gdma_ctx[cur_id].me1_addr_offset, me1_size);
        addr_offset += me1_size;
      ...
      if (!mem0) {
        GST_ERROR_OBJECT(thiz, "Failed to allocate YUV DMA buffer memory");
        if (yuv_buf.me0_ctx[cur_id].me_dma_buf_fd >= 0) {
          close(yuv_buf.me0_ctx[cur_id].me_dma_buf_fd);
          yuv_buf.me0_ctx[cur_id].me_dma_buf_fd = -1;
          yuv_buf.me1_ctx[cur_id].me_dma_buf_fd = -1;
      ...
      if (thiz->capture_me0) {
        ...
        if (!mem1) {
          GST_ERROR_OBJECT(thiz, "Failed to allocate ME0 DMA buffer memory");
          if (yuv_buf.me0_ctx[cur_id].me_dma_buf_fd >= 0) {
            close(yuv_buf.me0_ctx[cur_id].me_dma_buf_fd);
            yuv_buf.me0_ctx[cur_id].me_dma_buf_fd = -1;
            yuv_buf.me1_ctx[cur_id].me_dma_buf_fd = -1;
      ...
      if (thiz->capture_me1) {
        if (!thiz->capture_me0) {
          ...
          if (!mem1) {
            GST_ERROR_OBJECT(thiz, "Failed to allocate ME1 DMA buffer memory");
            if (yuv_buf.me1_ctx[cur_id].me_dma_buf_fd >= 0) {
              close(yuv_buf.me1_ctx[cur_id].me_dma_buf_fd);
              yuv_buf.me0_ctx[cur_id].me_dma_buf_fd = -1;
              yuv_buf.me1_ctx[cur_id].me_dma_buf_fd = -1;
            }
            gst_buffer_unref (p_out_buf);
...
static GstCaps *
gst_amba_camsrc_get_caps (GstBaseSrc * bsrc, GstCaps * filter)
{
  GstAmbaCamsrc *src = GST_AMBA_CAMSRC (bsrc);
