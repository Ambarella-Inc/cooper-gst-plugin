...
#endif
#ifndef YOLOV5_MULTI_CLASS_PER_ANCHOR
#define YOLOV5_MULTI_CLASS_PER_ANCHOR   0
#endif
...
int yolov5_post_process(void *context)
{
    int rval = 0;
    ...
    int class_num = ctx->output_cfg.out_desc[0].dim.depth / YOLOV5_ANCHOR_NUM - 5;
    ...
    do {
        ...
        for (m = 0; m < feature_map_num; m++) {
            ...
            if (ctx->output_cfg.out_desc[m].data_fmt.size == 1) {
                ...
                feature_map_data = ctx->nn_out_f32;
