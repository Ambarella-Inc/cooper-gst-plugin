 ...
 *
 * YOLOv3 (tiny) post-processing for mlpostprocess.
 */
...
#endif
/* Anchors for yolov3_fastest (layer115-conv, layer125-conv). Different from official
 * Darknet yolov3-tiny: 13x13 uses (81,82),(135,169),(344,319); 26x26 uses (10,14),(23,27),(37,58).
 * Order: FM0=coarser grid (e.g. 13x13), FM1=finer grid (e.g. 26x26). */
static float const tiny_yolov3_anchors[TINY_YOLOV3_FEATURE_MAP_NUM][TINY_YOLOV3_ANCHOR_NUM][2] = {
    {{189, 126}, {137, 236}, {265, 259}},
    {{26, 48}, {67, 84}, {72, 175}}
};
...
int mlpp_yolov3_post_process(void *context)
{
  ...
  int det_idx[TINY_YOLOV3_FEATURE_MAP_NUM];
  int class_num;
  int height[TINY_YOLOV3_FEATURE_MAP_NUM];
  int width[TINY_YOLOV3_FEATURE_MAP_NUM];
  ...
  /* Index-based; sort by spatial size ascending to tolerate out_name order */
  mlpp_sort_indices_by_spatial_asc(ctx, TINY_YOLOV3_FEATURE_MAP_NUM, det_idx);
  class_num = ctx->tensors[det_idx[0]].depth / TINY_YOLOV3_ANCHOR_NUM - 5;
...
/* Built-in YOLOv3 tiny ops for 2-tensor models. */
static const ml_postproc_output_pad_spec_t s_yolov3tiny_builtin_pads[] = {
  { .name = "bbox", .kind = ML_POSTPROC_PAD_BBOX },
};
static const ml_postproc_output_pad_spec_t *yolov3tiny_builtin_get_pads(int *count) {
  *count = 1;
  return s_yolov3tiny_builtin_pads;
}
static const ml_postproc_ops_t mlpp_yolov3tiny_builtin_ops = {
  .name = "yolov3tiny",
  .description = "YOLOv3 tiny built-in (2 tensors: layer115-conv, layer125-conv)",
  .process = mlpp_yolov3_post_process,
  .get_result_types = NULL,
  .get_output_pads = yolov3tiny_builtin_get_pads,
  .get_output_layout = NULL,
  .deinit_user_ctx = NULL,
  .output_coords_normalized = FALSE,
};
void ml_register_yolov3_builtin(void)
{
  ml_register_postproc("yolov3tiny", &mlpp_yolov3tiny_builtin_ops);
}
